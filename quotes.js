...
module.exports = bot => {
  ...
  "Cukup antartika aja yang jauh. Antarkita jangan."
    ...
    bot.on(['/katabijak'], async (msg, args) => {
                body = fs.readFileSync('./lib/katabijak.txt', 'utf-8')
                splitnix = body.split('\n')
                randomnix = splitnix[Math.floor(Math.random() * splitnix.length)]
                bot.sendAction(msg.chat.id, 'typing')
        return await bot.sendMessage(msg.chat.id, randomnix)
    })
    bot.on(['/toxic', '/badword'], async (msg, args) => {
    kata = [
        'babi',
        'monyet',
        'anjing',
    ...
    randKata = kata[Math.floor(Math.random() * kata.length)]
    list = [
    ...
    let toxic = list[Math.floor(Math.random() * list.length)]
    bot.sendAction(msg.chat.id, 'typing')
        return await bot.sendMessage(msg.chat.id, toxic, { replyToMessage: msg.message_id})
    })
    ...
    bot.on('/quotes', async (msg) => {
    
    data = fs.readFileSync('./lib/quote.json');
                 jsonData = JSON.parse(data);
                 randIndex = Math.floor(Math.random() * jsonData.length);
                 randKey = jsonData[randIndex];
            var author = randKey.by
            var kata = randKey.quote
